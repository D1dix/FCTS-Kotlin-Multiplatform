...
class CommentService(private val client: HttpClient, private val baseUrl: String) {
    ...
    suspend fun crearComentario(
        ...
        return try {
            ...
            println("🔑 [Service] Token usado: $authToken")
            ...
            println("📥 [Service] Response: ${response.bodyAsText()}")
    ...
    suspend fun puedeComentar(comida: String, restaurante: String, token: String): Boolean {
        println("📡 [Service] Verificando si puede comentar: $comida en restaurante: $restaurante")
        ...
        return try {
            ...
            if (response.status == HttpStatusCode.OK) {
                ...
                println("✅ [Service] Resultado de puedeComentar: $result")
                result
            } else {
                println("❌ [Service] Error en la respuesta: ${response.status}")
                false
            }
        } catch (e: Exception) {
            println("❌ [Service] Error en la petición puedeComentar: ${e.message}")
            e.printStackTrace()
            false
        }
    }
